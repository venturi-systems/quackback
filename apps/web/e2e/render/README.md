...
- `keyboard/<route>__<context>.json`: every stop of both walks, with its findings. The context names the viewport and the motion setting, such as `phone-coarse-motion`. The reverse walk is compared with the forward one by element, not by selector, and the elements only one walk reached are listed (`forwardOnly`, `reverseOnly`). A list that loads another page while the walk passes it adds stops on the way back, so a difference is evidence for review, not a failure.
- `review/<route>__<width>__spacing.json`: the text-spacing captures of the authored review items, taken after the reduced-motion walk, each region with its box on the page and its recorded resolution. The captures are taken before the checker runs, so a region's disposition says only whether a resolution is on record for its route, text and width. The summary's text-spacing capture table says whether the checker flagged each region on the same run.
- `summary.md`: the job summary.
- `server.log`: the app's log.

The job fails on any checker `FAIL`, on any checker run that ended on another path than the planned one (a redirect, such as to sign-in when a session was not honoured, would otherwise measure the wrong page and pass), on any keyboard finding, or on any authored review item with no recorded resolution. A checker `NEEDS_REVIEW` does not by itself fail the job. The summary lists each one: prose, user-generated text, the text-spacing stress run, font evidence or unmeasured text. Each needs its own reviewed resolution. For authored text it is recorded in `review-resolutions.ts` after the render is inspected, and the summary prints it beside the item, or `none recorded` when the item's route, text or width has none; a `none recorded` item fails the job, so a new or changed item is inspected before the job passes again. User-generated text keeps the suite's own disposition: its words stay as written. A resolution must hold on every run: the seed gives each board a different post count on each run, so a resolution never states one run's count or one run's line break as if it always rendered that way.
