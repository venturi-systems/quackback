...
function MinimalDocument({ children }: Readonly<{ children: ReactNode }>) {
  // No route context here. The Venturi fallback document is always light (its
  // data-theme and color-scheme meta say so), so commit the light class and
  // color-scheme on the SSR <html> too rather than upstream's OS-driven
  // `system` canvas, which would paint dark behind a light page.
  const { className: themeClass, colorScheme } = resolveDocumentTheme('light')
  return (
    <html
      lang="en"
      className={themeClass}
      data-theme="light"
      data-venturi-web-theme="light"
      style={{ colorScheme }}
